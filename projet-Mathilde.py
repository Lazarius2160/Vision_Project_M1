...
giCapteur = -1                                    # ID du capteur de proximite de Roburrin
giSensor = -1                                     # ID du capteur de proximite de Kuka
giTarget = -1                                     # ID du target dummy
giBase = -1       
giTool = -1                                       # ID de la base mobile
giObject = -1 
gModelIsDynamic = 0
iCount = 0
...
# a ultrasons
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
def GetDistanceMeasurement(Robot):
    if Robot == "Sensor_roburrin" :
...
def Go5( dbDist, dbVmax, dbEpsilon, dbGain, dbMinDist, dbTimeStep):
  ...
  while abs( dbDparcourue - abs(dbDist)) > dbEpsilon:
      ...
      bResult, dbDistTarget = GetDistanceMeasurement("Sensor_roburrin")
      if bResult == False:
...
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
# Faire tourner sur lui-même le robot
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
def BourineCylindre2( dbAngleStep, dbGoStep, dbMinDist,bGoR):
    ...
    while dbDist > dbMinDist:
        ...
        if iResult > 0:
            Turn2((dbAz/180.0)*math.pi, 0.2, 0.05, 0.1, 0.1)    #JBO : gain passe de 0.5 a 0.1
            # on recale la camera dans l'axe d'avance
            SetCamOrientation(gsiID, giCamMotor, math.pi)
            # estimation de la distance a la cible : 
            GetDistanceMeasurement("Sensor_roburrin")
            ...
            iError, dbDistTarget = GetDistanceMeasurement("Sensor_roburrin")
            if iError == False:
                dbDistTarget = dbDEFAULT_DISTANCE
...
#...................................
# recuperation du handle sur le tool 
#...................................
...
if( siErrorCode != sim.simx_error_noerror ):
  ...
  sim.simxFinish(siID)
  exit()
print("lien au tool OK (ID = " + str(giTarget) + ")")
#...............................................
...
#On fait un demi-tour pour donner accès au kuka
#Turn2(math.pi,0.5,0.05,0.5,0.3)
Turn2(math.pi,0.5,0.05,0.2,0.1)
#On recule d'0.7m pour s'approcher du cylindre
#Go5(0.7,0.5,0.05,0.3,0.1,0.3)
Go5(0.7,0.5,0.05,0.3,0.1,0.1)
print("En position, début de capture")
# deplacement de l'outil
# JBO : on peut se permettre de "tricher" un peu en recuperant directement la position du cube a capturer 
szTOY = "toy"
siErrorCode, giToy = sim.simxGetObjectHandle(siID, szTOY, sim.simx_opmode_blocking)
if( siErrorCode != sim.simx_error_noerror ):
  print('ERREUR : main() ---> apppel a simxGetObjectHandle()\n')
  print('         code de retour V-REP = ' + str(siErrorCode))
  sim.simxFinish(siID)
  exit()
print("lien au cube a capturer OK (ID = " + str(giToy) + ")")
#Pos_test,Ori_test=GetObjectRelativePositionAndOrientation(giCylindre,-1)
Pos_test,Ori_test=GetObjectRelativePositionAndOrientation(giToy,-1)
# on doit se placer juste au dessus de la surface du cube :
Pos_test[2]+= 0.3  #on se prepositionne 30 cm au dessus du cube
PlaceToolAt( -1, Pos_test) 
# on redescend pour n'etre plus qq cmd
Pos_test[2]-= 0.18
#Pos_test[2]+=0.15  # 15 cm au dessus du centre du cube pour declencher la detection et la capture
#Pos_test[-1]+=0.50
#print(Pos_test,Ori_test)
#MoveToolTo(-1, Pos_test,Ori_test)
PlaceToolAt( -1, Pos_test)
print("Etape 1")
#PTP(Pos_test, 0.1, 0.1,-1)
#print("Etape 2")
Capture()
# on remonte de 30 cm avec le cube
Pos_test[2]+= 0.3
PlaceToolAt( -1, Pos_test)
#print("Etape 3")
#PTP(Pos_test, 0.1, 0.1, -1)
#print("Etape 4")
#PTP([xTest+0.1,yTest,zTest+0.4], 0.1, 0.1,giCapteur)
#print("Etape 5")
#PTP([xTest+0.1,yTest,zTest], 0.1, 0.1, giCapteur)
# print("Etape 6")
# Release()
# print("Etape 7")
# PTP([xTest+0.1,yTest,zTest+0.4], 0.1, 0.1, giCapteur)
