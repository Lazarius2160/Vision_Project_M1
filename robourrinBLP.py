...
def GetDistanceMeasurement():
    iError,bDetected,Point,iObject,surface=sim.simxReadProximitySensor(gsiID,giCapteur,sim.simx_opmode_blocking)
    if bDetected:
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
# commande de rotation sur place (meme principe que Go2)
# -----------------------------------------------------------
# AJOUTER SOUS VREP LE TRACE DE LA TRAJECTOIRE AU SOL ET
# DEMANDER AU ROBOT DE DECRIRE UN CARRE...
# (penser a modifier la taille du buffer associe au graphe 
# pour obtenir une trace assez longue : 1 000 000 de points)
# ---> faire constater aux eleves que le carre n'est pas parfait
...
#             du robot).                                
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
def GetMobileBasePosition2(iRef):  
  #............................
  # recuperation de la position
  #............................
...
def PTP( tPos, dbVel, dbTimeStep, iRef):
    ...
    print("PTP")
    while True:
        cPos, cOri = GetMobileBasePosition2(iRef)
...
def Capture():
    ...
    if giObject == -1:
        ...
        iState, dbDist, giObject = GetDistanceMeasurement2()
        print(iState)
        print(giObject)
...
if( siErrorCode != sim.simx_error_noerror ):
  print('ERREUR : main() ---> apppel a simxGetObjectHandle() Base mobile\n')
...
if( siErrorCode != sim.simx_error_noerror ):
  print('ERREUR : main() ---> apppel a simxGetObjectHandle() Moteur gauche\n')
  print('         code de retour V-REP = ' + str(siErrorCode))
  sim.simxFinish(siID)
  exit()
...
if( siErrorCode != sim.simx_error_noerror ):
  print('ERREUR : main() ---> apppel a simxGetObjectHandle() Moteur droit\n')
  print('         code de retour V-REP = ' + str(siErrorCode))
  sim.simxFinish(siID)
  exit()
...
if( siErrorCode != sim.simx_error_noerror ):
  print('ERREUR : main() ---> apppel a simxGetObjectHandle() Moteur tourelle\n')
  print('         code de retour V-REP = ' + str(siErrorCode))
  sim.simxFinish(siID)
  exit()
...
if( siErrorCode != sim.simx_error_noerror ):
  print('ERREUR : main() ---> apppel a simxGetObjectHandle() Camera \n')
  print('         code de retour V-REP = ' + str(siErrorCode))
  sim.simxFinish(siID)
  exit()
...
#...............................................
siErrorCode, giCylindreR = sim.simxGetObjectHandle(siID, CYLINDRER, sim.simx_opmode_blocking)
if( siErrorCode != sim.simx_error_noerror ):
    print('ERREUR : main() ---> apppel a simxGetObjectHandle() Collision \n')
    print('         code de retour V-REP = ' + str(siErrorCode))
    sim.simxFinish(siID)
    exit()
print("lien a la collision OK (ID = " + str(giCylindreR) + ")")
#...............................................
# recuperation du handle sur l'objet à attraper "toy"
#...............................................
siErrorCode, iToy = sim.simxGetObjectHandle(siID, TOY, sim.simx_opmode_blocking)
if( siErrorCode != sim.simx_error_noerror ):
    print('ERREUR : main() ---> apppel a simxGetObjectHandle() Toy\n')
    print('         code de retour V-REP = ' + str(siErrorCode))
    sim.simxFinish(siID)
    exit()
print("lien a la collision OK (ID = " + str(iToy) + ")")
giToy = iToy
#...............................................
# recuperation du handle "tool"
#...............................................
siErrorCode, iTool = sim.simxGetObjectHandle(siID, TOOL, sim.simx_opmode_blocking)
if( siErrorCode != sim.simx_error_noerror ):
    print('ERREUR : main() ---> apppel a simxGetObjectHandle() Tool\n')
    print('         code de retour V-REP = ' + str(siErrorCode))
    sim.simxFinish(siID)
    exit()
print("lien a la collision OK (ID = " + str(iTool) + ")")
giTool = iTool
#...........................................
...
if( siErrorCode != sim.simx_error_noerror ):
  print('ERREUR : main() ---> apppel a simxGetObjectHandle() target dummy\n')
  print('         code de retour V-REP = ' + str(siErrorCode))
  sim.simxFinish(siID)
  exit()
...
if iVERSION > 2:
    ...
    #......................................
    siErrorCode, giCapteur = sim.simxGetObjectHandle(siID, CAPTEUR, sim.simx_opmode_blocking)
    if( siErrorCode != sim.simx_error_noerror ):
        print('ERREUR : main() ---> apppel a simxGetObjectHandle() Capteur\n')
        print('         code de retour V-REP = ' + str(siErrorCode))
        sim.simxFinish(siID)
        exit()
    print("lien au capteur OK (ID = " + str(giCapteur) + ")")
#...............................................
...
# Déplacement de Target au niveau de Tool :
PosToolR , OriTool = GetObjectRelativePositionAndOrientation(giTool, iBaseHandle)
iError = sim.simxSetObjectPosition(gsiID,giTarget,iBaseHandle,PosToolR,sim.simx_opmode_blocking)
print ("Placement de target")
iError, PosToy = sim.simxGetObjectPosition(gsiID,giToy,-1,sim.simx_opmode_blocking)
PosToy[2] += 0.07
...
if iVERSION < 3 :
    ...
    # on fait un demi tour
    Turn2((180/180.0)*math.pi, 0.5, 0.05, 0.1, 0.1)
    Go3(0.7, 0.5, 0.02, 0.15, 0.1)
    time.sleep(1)
    # on repère l'objet target et on le prend
    PTP(PosToy, 0.2, 0.1, -1)
    Capture()
    PTP(PosToolR, 0.2, 0.1, iBaseHandle)
    # puis recherche et rencontre du cylindre rouge :
    ...
    Go3(0.7, 0.5, 0.02, 0.15, 0.1)
    #on dépose l'objet
    iError, PosCylindreR = sim.simxGetObjectPosition(gsiID,giCylindreR,-1,sim.simx_opmode_blocking)
    PosCylindreR[2]+=0.445
    PTP(PosCylindreR, 0.2, 0.1 , -1)
    Release()
    time.sleep(1)
    PTP(PosToolR, 0.2, 0.1, iBaseHandle)
